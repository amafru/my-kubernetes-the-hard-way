# Ama's Kubernetes The Hard Way Repo

Updated: March 2024

This tutorial walks you through setting up Kubernetes the hard way on a local machine using a hypervisor.

Kubernetes The Hard Way is optimized for gaining a granular understanding of all the key cluster components, which means taking the long route to ensure you understand each task required to bootstrap a Kubernetes cluster. Note that the cluster when built will not be accessible from your laptop browser - that isn't what this repo is about. 

The repo is my modified version of Mumshad Manambeth's *-the-hard-way repo (https://github.com/mmumshad/kubernetes-the-hard-way) which is itself a fork of the original developed by [Kelsey Hightower](https://github.com/kelseyhightower/kubernetes-the-hard-way).

While the original one uses GCP as the platform to deploy kubernetes,  this one uses a hypervisor to deploy a cluster on a local machine. If you prefer the cloud version, refer to the original one [here](https://github.com/kelseyhightower/kubernetes-the-hard-way)

The results of this tutorial should *not* be viewed as production ready, and may receive limited support from the community, but it's still very useful for learning!
